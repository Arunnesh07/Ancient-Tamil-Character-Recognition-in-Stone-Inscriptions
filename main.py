...
input_file="sentence.txt" #input sentence should not have any leading empty lines
pos_tagged_file="pos-tagged.txt"
tamil_tagged_file="tamil-tagged"
pos_tamil_aligned_file="pos-tamil-aligned.txt"
unknown_tamil_file="errors-unknown.txt"
...
#this needs to be passed to the function using which tamilemes are extracted
fsts=[]
...
pos_tagged=open(pos_tagged_file, 'w')
pos_tamil_aligned=open(pos_tamil_aligned_file, 'w')
tamil_tagged=open(tamil_tagged_file, 'w')
unknown_tamil=open(unknown_tamil_file,'w')

pos_tamil_aligned.write("#This POS-tamilology tagged file was annotated using Thamizhitamil tamilological analyser (Python version) \n \
#This is formatted in CoNLL-U format \n \
#More information about Thamizhitamil can be found here: http://nlp-tools.uom.lk/thamizhi-tamil/ \n \
#Tagged on "+ str(datetime.datetime.now()) +" \n")
pos_tagged.write("#This POS tagged file was annotated using ThamizhiPOSt  -Tamil POS tagger \n \
#More information about ThamizhiPOSt can be found here: http://nlp-tools.uom.lk/thamizhi-pos/ \n \
#Tagged on "+ str(datetime.datetime.now()) +" \n")
tamil_tagged.write("#This POS-tamilology tagged file was annotated using Thamizhitamil tamilological analyser (Python version) \n \
#More information about Thamizhitamil can be found here: http://nlp-tools.uom.lk/thamizhi-tamil/ \n \
#Tagged on "+ str(datetime.datetime.now()) +" \n")
...
for data_unit in data_input:
        ...
        for sent in doc.sentences :
                ...
                for word in sent.words :
                        pos_tagged.write(word.text + "\t" + word.upos + "\n")
                        #print(word.text)
                        #this is set to 0, and when an analysis is found, it will be set to 1                  
                        analysis_success=0
                        #print(word.text)
                        #All the Puncts are marked here with PUNCT. There are no analysis for PUNCT from Thamizhitamil
                        #if word.upos == "PUNCT" :
                        #        pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + word.text + "\t" + word.upos+ "\t" + "_" + "\n")
                        if word.text in punct_json:
                                pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + "_" + "\t" + "PUNCT" + "\t" + punct_json[word.text] + "\n")                                
                        #All numeric will take default annotations given by the POS tagger
                        elif word.text.isnumeric() :
                                pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + word.text + "\t" + word.upos+ "\t" + "_" + "\n")
                        elif isEnglish(word.text) == 1 :
                                pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + "non-Tamil" + "\t" + "_" + "\t" + "_" + "\n")
                        else:
                                #if a word is found in lexicons
                                analyses = find_tamilemes(word.text.strip(),fsts)
                                ...
                                if analyses != 0 :
                                        for each_analysis in analyses:
                                                ...
                                                counter=0
                                                #print(len(analyses))
                                                tamil_tagged.write(",".join(each_analysis)+"\n")
                                                ...
                                                if pos_tamil==1:
                                                        ...
                                                        pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + lemma + "\t" + word.upos+ "\t" +  '-'.join(list(set(lables.split(","))))  + "\n")
                                                        break;
                                                else:
                                                        if lables=="":
                                                                lables="_"
                                                        pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + lemma + "\t" + pos.upper()+ "\t" + '-'.join(list(set(lables.split(",")))) + "\n")
                                                        unknown_tamil.write(str(word_id) + "\t" + word.text + "\t" + word.upos+ "\t" + lemma + "\t" + pos + "\t" + '-'.join(list(set(lables.split(",")))) + "\n")
                                        ...
                                        if word.upos == "ADJ":
                                                ...
                                                pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + "_" + "\t" + word.upos+ "\t" + "_" + "\n")
                                                unknown_tamil.write(str(word_id) + "\t" + word.text + "\t" + word.upos+ "\t" + "_" + "\t" + "_" + "\n")
                                                word_id=word_id+1
                                                continue
                                                
                                        gusses = guess_tamilemes(word.text.strip(),guesser_list)
                                        ...
                                        if gusses != 0 :
                                                for each_guess in gusses:
                                                        ...
                                                        lables=""
                                                        pos=""
                                                        tamil_tagged.write(",".join(each_guess)+"\n")
                                                        for analysis in each_guess:
                                                                ...
                                                                if counter==0:
                                                                        ...
                                                                        #        lables=analysis
                                                                        #else:
                                                                                #if the analysis has =, which means if tamil information are there.
                                                                                if "=" in analysis:
                                                                                        lable_new=analysis.split("=")[0]
                                                        ...
                                                        #this piece is to compare upos and the pos from Thamizhitamil
                                                ...
                                                if lables=="":
                                                        lables="_"
                                                pos_tamil_aligned.write(str(word_id) + "\t" + word.text + "\t" + "Unknown" + "\t" + word.upos+ "\t" + "Unknown" + "\n")
                                                unknown_tamil.write(str(word_id) + "\t" + word.text + "\t" + "Unknown" + "\t" + word.upos+ "\t" + "Unknown" + "\n")
                        
                        tamil_tagged.write("\n")                
                        #this is a word counter
                        #if(analysis_success !=0) :
                        word_id=word_id+1
pos_tamil_aligned.close()
pos_tagged.close()
tamil_tagged.close()
unknown_tamil.close()
